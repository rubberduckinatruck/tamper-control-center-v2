...
//
// @cc-id              chatgpt-exporter
// @cc-display-name    ChatGPT Exporter
// @cc-category        gpt
// @cc-role            personal
// @cc-status          live
// @cc-tags            chatgpt, conversations, history, export, backup
// @cc-note            Third-party MIT-licensed userscript by pionxzh.
//
...
// @run-at             document-end
//
// @updateURL          https://raw.githubusercontent.com/rubberduckinatruck/tamper-control-center-v2/main/scripts/gpt/chatgpt-exporter.user.js
// @downloadURL        https://raw.githubusercontent.com/rubberduckinatruck/tamper-control-center-v2/main/scripts/gpt/chatgpt-exporter.user.js
// ==/UserScript==
...
(function (JSZip) {
  ...
  const KEY_PROJECT_CHAT_COUNTS = "exporter:project_chat_counts";
  ...
  async function fetchConversations(offset = 0, limit = 20, project = null) {
    if (Array.isArray(project)) {
      const projectList = project.filter(Boolean);
      project = projectList.length === 1 ? projectList[0] : null;
    }
    if (project) {
  ...
  async function fetchAllConversations(project = null, maxConversations = 1e3, onBatch, onHasMore) {
    if (Array.isArray(project)) {
      ...
      if (projectList.length === 0) {
        return fetchAllConversations(null, maxConversations, onBatch, onHasMore);
      }
      const seen = /* @__PURE__ */ new Set();
      const combined = [];
      const maxTotal = maxConversations * projectList.length;
      for (const projectId of projectList) {
        if (combined.length >= maxTotal) break;
        await fetchAllConversations(projectId, maxConversations, (batch) => {
          const filteredBatch = batch.filter((c2) => !seen.has(c2.id));
          filteredBatch.forEach((c2) => seen.add(c2.id));
          if (filteredBatch.length > 0) {
            combined.push(...filteredBatch);
            onBatch == null ? void 0 : onBatch(filteredBatch);
          }
        }, void 0);
      }
      const result = combined.slice(0, maxTotal);
      onHasMore == null ? void 0 : onHasMore(false);
      return result;
    }
    const conversations = [];
...
\f\r "&'/<=>`.split(""), "\0	\n\f\r \"&'/<=>`".split("")]
  ...
  async function exportAllToHtml(fileNameFormat, apiConversations, metaList, projectName, partIndex, totalParts) {
    const userAvatar = await getUserAvatar();
    const zip = new JSZip();
    if (currentPartialExportManifest) {
      zip.file("_export_progress_manifest.json", JSON.stringify(currentPartialExportManifest, null, 2));
    }
...
${content2}`;
  ...
  let currentPartialExportManifest = null;
  const EXPORT_PACING_BATCH_SIZE = 5;
  const EXPORT_DELAY_BETWEEN_CHATS_MIN_MS = 8e3;
  const EXPORT_DELAY_BETWEEN_CHATS_MAX_MS = 12e3;
  const EXPORT_BATCH_COOLDOWN_MIN_MS = 25e3;
  const EXPORT_BATCH_COOLDOWN_MAX_MS = 45e3;
  const EXPORT_RATE_LIMIT_EXTRA_MIN_MS = 2e3;
  const EXPORT_RATE_LIMIT_EXTRA_MAX_MS = 10e3;
    ...
    async process() {
      ...
      if (remaining > 0) {
        while (Date.now() < this.pauseUntil) {
          const waitSecs = Math.ceil((this.pauseUntil - Date.now()) / 1e3);
          this.progress(this.queue[0].name, "rate_limited", waitSecs);
          await sleep(Math.min(1e3, Math.max(0, this.pauseUntil - Date.now())));
        }
        this.pauseUntil = 0;
      }
      ...
      try {
        ...
        if (error2 instanceof RateLimitError) {
          ...
          ) + randomIntBetween(EXPORT_RATE_LIMIT_EXTRA_MIN_MS, EXPORT_RATE_LIMIT_EXTRA_MAX_MS);
      ...
      if (waitMs > 0) {
        await sleep(waitMs);
      } else if (this.queue.length > 0 && this.status !== "STOPPED") {
        const isBatchCooldown = this.completed > 0 && this.completed % EXPORT_PACING_BATCH_SIZE === 0;
        const pacingMs = isBatchCooldown
          ? randomIntBetween(EXPORT_BATCH_COOLDOWN_MIN_MS, EXPORT_BATCH_COOLDOWN_MAX_MS)
          : randomIntBetween(EXPORT_DELAY_BETWEEN_CHATS_MIN_MS, EXPORT_DELAY_BETWEEN_CHATS_MAX_MS);
        const pacingStatus = isBatchCooldown ? "batch_cooldown" : "delay_before_next_chat";
        const pacingUntil = Date.now() + pacingMs;
        while (Date.now() < pacingUntil) {
          const waitSecs = Math.ceil((pacingUntil - Date.now()) / 1e3);
          this.progress(this.queue[0].name, pacingStatus, waitSecs);
          await sleep(Math.min(1e3, Math.max(0, pacingUntil - Date.now())));
        }
      }
      this.process();
    }
  ...
  function readProjectChatCounts() {
    try {
      const parsed = JSON.parse(localStorage.getItem(KEY_PROJECT_CHAT_COUNTS) || "{}");
      return parsed && typeof parsed === "object" ? parsed : {};
    } catch {
      return {};
    }
  }
  function writeProjectChatCounts(counts) {
    try {
      localStorage.setItem(KEY_PROJECT_CHAT_COUNTS, JSON.stringify(counts || {}));
    } catch (error2) {
      console.warn("Could not save project chat counts", error2);
    }
  }
  function formatProjectNameWithCount(project, projectCounts) {
    const name = project?.display?.name || "Untitled Project";
    const count = projectCounts ? projectCounts[project.id] : void 0;
  ...
  const ProjectSelect = ({ projects, selected, setSelected, disabled, loading, projectCounts = {} }) => {
    ...
    return /* @__PURE__ */ o$8("div", { className: "ProjectSelect flex items-center text-gray-600 dark:text-gray-300 justify-between mb-3", children: [
      ...
      /* @__PURE__ */ o$8("div", { className: "flex items-center gap-2", children: [
        ...
        /* @__PURE__ */ o$8(
          ...
          {
            multiple: true,
            size: Math.min(8, Math.max(3, projects.length + 1)),
            disabled,
            className: "Select",
            value: selectedValues,
            title: "Select multiple projects with Ctrl/Cmd-click or Shift-click. Leave blank for all conversations.",
            onChange: (e2) => {
              const vals = Array.from(e2.currentTarget.selectedOptions).map((o3) => o3.value).filter(Boolean);
              setSelected(vals);
            },
            children: [
              /* @__PURE__ */ o$8("option", { value: "", children: t2("All conversations") }),
              projects.map((project) => /* @__PURE__ */ o$8("option", { value: project.id, children: formatProjectNameWithCount(project, projectCounts) }, project.id))
            ]
          }
        )
      ] })
  ...
  const DialogContent = ({ format }) => {
    ...
    const [selectedProjectId, setSelectedProjectId] = h$4([]);
    const [projectsLoading, setProjectsLoading] = h$4(false);
    const [projectChatCounts, setProjectChatCounts] = h$4(() => readProjectChatCounts());
    const selectedProjectIds = Array.isArray(selectedProjectId) ? selectedProjectId : selectedProjectId ? [selectedProjectId] : [];
    const selectedProjects = projects.filter((p2) => selectedProjectIds.includes(p2.id));
    const selectedProject = selectedProjects.length === 1 ? selectedProjects[0] : null;
    const selectedProjectName = selectedProjects.length > 1 ? `${selectedProjects.length} Projects` : selectedProject == null ? void 0 : selectedProject.display.name;
    ...
    const [exportType, setExportType] = h$4("JSON (ZIP)");
    ...
    const exportTotalRef = _(0);
    const completedResultsRef = _([]);
    const pausedRef = _(false);
    ...
    p$6(() => {
      const off = requestQueue.on("progress", (prog) => {
        ...
        setProgress({
          ...
          completed: Math.min(exportTotalRef.current || selected.length || prog.total, batchIndexRef.current * EXPORT_OPERATION_BATCH + prog.completed),
          total: exportTotalRef.current || selected.length || prog.total
        });
      });
      return () => off();
    }, [requestQueue, selected]);
    p$6(() => {
      const off = requestQueue.on("result", (result) => {
        completedResultsRef.current = [...completedResultsRef.current, result];
      });
      return () => off();
    }, [requestQueue]);
    p$6(() => {
    ...
    const pauseExport = T$4(() => {
      pausedRef.current = true;
      requestQueue.pause();
    }, [requestQueue]);
    const downloadProgress = T$4(async () => {
      var _a;
      const completed = completedResultsRef.current || [];
      if (!completed.length) {
        alert("No completed chats are available to download yet.");
        return;
      }
      const callback = (_a = exportAllOptions.find((o3) => o3.label === exportType)) == null ? void 0 : _a.callback;
      if (!callback) return;
      currentPartialExportManifest = {
        partial_export: true,
        exported_at: new Date().toISOString(),
        completed_count: completed.length,
        selected_count: selected.length,
        pending_count: Math.max(0, selected.length - completed.length),
        export_type: exportType,
        selected_project: selectedProjectName
      };
      try {
        await callback(format, completed, metaList, selectedProjectName, 1, 1);
      } finally {
        currentPartialExportManifest = null;
      }
    }, [exportAllOptions, exportType, format, metaList, selected.length, selectedProjectName]);
    const exportAllFromApi = T$4(() => {
      ...
      exportTotalRef.current = selected.length;
      setProcessing(true);
      setProgress({
    ...
    p$6(() => {
      ...
      fetchAllConversations(
        ...
        if (!alive()) return;
        const ids = Array.isArray(selectedProjectId) ? selectedProjectId.filter(Boolean) : selectedProjectId ? [selectedProjectId] : [];
        if (ids.length > 0 && Array.isArray(loadedConversations)) {
          setProjectChatCounts((prev) => {
            const next = { ...prev };
            if (ids.length === 1) {
              next[ids[0]] = loadedConversations.length;
            } else {
              ids.forEach((id) => {
                const count = loadedConversations.filter((c2) => c2.gizmo_id === id || c2.gizmoId === id || c2.gizmo?.id === id).length;
                if (count > 0) next[id] = count;
              });
            }
            writeProjectChatCounts(next);
            return next;
          });
        }
      }).catch((err) => {
    ...
    return /* @__PURE__ */ o$8(k$3, { children: [
      ...
      /* @__PURE__ */ o$8("div", { className: "flex items-center text-gray-600 dark:text-gray-300 flex justify-between border-b-[1px] pb-3 mb-3 dark:border-gray-700", style: { display: "none" }, children: [
      ...
      exportSource === "API" && /* @__PURE__ */ o$8(
        ProjectSelect,
        {
          ...
          projectCounts: projectChatCounts
        }
      ),
      /* @__PURE__ */ o$8(
        ConversationSelect,
      ...
      processing && /* @__PURE__ */ o$8(k$3, { children: [
        /* @__PURE__ */ o$8("div", { className: "mt-2 mb-1 justify-between flex items-center gap-2", style: { paddingRight: "260px" }, children: [
          /* @__PURE__ */ o$8("span", { className: "truncate text-sm text-gray-600 dark:text-gray-300", children: progress.currentStatus === "rate_limited" ? `⏳ Rate limited — ${progress.rateLimitWaitSecs ?? "…"}s` : progress.currentStatus === "batch_cooldown" ? `Cooldown — ${progress.rateLimitWaitSecs ?? "…"}s` : progress.currentStatus === "delay_before_next_chat" ? `Next chat — ${progress.rateLimitWaitSecs ?? "…"}s` : progress.currentName }),
      ...
      processing ? /* @__PURE__ */ o$8(k$3, { children: [
        ...
        /* @__PURE__ */ o$8(
          "button",
          {
            ...
            title: "Pause after the current chat finishes",
            onClick: pauseExport,
            children: "Pause"
          }
        ),
        /* @__PURE__ */ o$8(
          "button",
          {
            ...
            title: "Download chats completed so far",
            ...
            /* @__PURE__ */ o$8($5d3850c4d0b4e6c7$export$7c6e2c02157bb7d2, { className: "DialogContent", children: [
              ...
              /* @__PURE__ */ o$8("div", { className: "DialogBody", children: [
                /* @__PURE__ */ o$8("dl", { className: "space-y-6", children: [
                  /* @__PURE__ */ o$8("div", { className: "relative flex bg-white dark:bg-white/5 rounded p-4", style: { display: "none" }, children: /* @__PURE__ */ o$8("div", { children: [
                    ...
                    children: [
                      /* @__PURE__ */ o$8(
                        SettingDialog,
                        {
                          ...
                          children: /* @__PURE__ */ o$8("div", { className: "row-half", children: /* @__PURE__ */ o$8(MenuItem, { text: t2("Setting"), icon: IconSetting }) })
                        }
                      ),
                      /* @__PURE__ */ o$8(
                        MenuItem,
